...
namespace eg
{
	...
	void EditorLayer::UI_Toolbar()
	{
		...
		float numButtons = 2;
		float totalWidth = (numButtons * size) + ((numButtons - 1) * 10.0f);
		float startPosX = (ImGui::GetWindowWidth() - totalWidth) * 0.5f;
	...
	const std::string EditorLayer::CompileCustomScripts()
	{
		...
		if (!pipe) {
			ConsolePanel::Log("Failed to open pipe for script compilation", ConsolePanel::LogType::Error);
			return "Failed to open pipe for script compilation due to system error";
		}
