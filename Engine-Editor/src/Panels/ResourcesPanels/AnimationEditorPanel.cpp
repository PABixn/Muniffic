...
namespace eg {
	...
	void AnimationEditorPanel::DrawPlayButton()
	{
		...
		if (ImGui::ImageButton("PlayOrStop##xx", (ImTextureID)(*m_Anim->IsPlayingPtr() ? IconLoader::GetIcon(Icons::Editor_StopButton)->GetRendererID() : IconLoader::GetIcon(Icons::Editor_PlayButton)->GetRendererID()), ImVec2(50, 50))) {
	...
	void AnimationEditorPanel::DrawActionButtons(const ImVec2& buttonSize, float gap)
	{
		...
		if (ImGui::ImageButton("LenghtChangeTool##xx", (ImTextureID)(IconLoader::GetIcon(Icons::AnimationEditor_LengthChange)->GetRendererID()), buttonSize, ImVec2(0, 0), ImVec2(1, 1), ImVec4(0, 0, 0, 0), resizeIconTint)) {
			if (m_Move)
				m_Move = false;
			m_Resize = !m_Resize;
		}
		ImGui::SetCursorScreenPos(ImVec2(ImGui::GetCursorScreenPos().x, ImGui::GetCursorScreenPos().y + gap));
		if (ImGui::ImageButton("MoveTool##xx", (ImTextureID)(IconLoader::GetIcon(Icons::AnimationEditor_MoveIcon)->GetRendererID()), buttonSize, ImVec2(0, 0), ImVec2(1, 1), ImVec4(0, 0, 0, 0), moveIconTint)) {
			if (m_Resize)
				m_Resize = false;
			m_Move = !m_Move;
		}
