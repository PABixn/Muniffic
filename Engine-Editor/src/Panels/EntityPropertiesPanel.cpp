...
namespace eg {
	...
	void EntityPropertiesPanel::DrawComponent(const std::string& name, Entity entity, UIFunction uiFunction, Ref<Scene>& context, Icons icon) {
		EG_PROFILE_FUNCTION();
	...
	template <typename T>
	void EntityPropertiesPanel::DisplayAddComponentEntry(const std::string& entryName) {
		EG_PROFILE_FUNCTION();
		if (!m_Entity.HasComponent<T>())
		{
			if (ImGui::MenuItem(entryName.c_str()))
			{
				Commands::ExecuteCommand<Commands::AddComponentCommand<T>>(Commands::CommandArgs("", m_Entity, m_Context, m_Entity));
				ImGui::CloseCurrentPopup();
			}
		}
	}
			...
			if (opened)
			{
				...
				for (auto& [className, value] : scriptMethods) {
					if (ImGui::Button(className.c_str())) {
						...
						data->ResourceName = className;
						data->Extension = ".cs";
						data->ParentDirectory = AssetDirectoryManager::GetRootAssetTypeDirectory(ResourceType::Script);
						data->Type = ResourceType::Script;

						uuid = ResourceDatabase::AddResource(AssetDirectoryManager::getDirectoryPath(data->ParentDirectory), data, ResourceType::Script);
						ImGui::CloseCurrentPopup();
						m_OpenFunctionPopupCall = false;
					}
				}
				ImGui::TreePop();
