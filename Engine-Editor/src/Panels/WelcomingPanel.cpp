...
{	
	...
	void WelcomingPanel::OnImGuiRender()
	{
		...
		if (m_ProjectList.size()) {
			for (auto project : m_ProjectList)
			{
				if (project != "")
				{
					...
					if (ImGui::ImageButton("WelcomePanel_DeleteProject##xx", (ImTextureID)IconLoader::GetIcon(Icons::WelcomePanel_DeleteProject)->GetRendererID(), ImVec2(30.0f, 30.0f), ImVec2(0, 0), ImVec2(1, 1))) {
					...
					ImGui::Image((ImTextureID)IconLoader::GetIcon(Icons::ContentBrowser_Directory)->GetRendererID(), { 32.0f, 32.0f }, ImVec2(0, 0), ImVec2(1, 1));
				}
				i++;
			}
		}
		...
		ImGui::Image((ImTextureID)IconLoader::GetIcon(Icons::ContentBrowser_Directory)->GetRendererID(), { 30.0f, 30.0f }, ImVec2(0, 0), ImVec2(1, 1));
		...
		ImGui::SameLine();
		ImGui::SetCursorPosX(posX);
		ImGui::Image((ImTextureID)IconLoader::GetIcon(Icons::WelcomePanel_NewProject)->GetRendererID(), { 32.0f, 32.0f }, ImVec2(0, 0), ImVec2(1, 1));
		ImGui::PopFont();
		ImGui::PopStyleVar();
