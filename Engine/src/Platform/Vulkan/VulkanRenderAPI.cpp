...
namespace eg {
    ...
    void VulkanRendererAPI::DrawIndexed(const Ref<VertexBuffer>& vertexBuffer, const Ref<IndexBuffer>& indexBuffer,const Ref<Shader>& shader)
    {
        vkCmdBindPipeline(m_CommandBuffer->getCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, ((VulkanShader&)(*shader)).GetGraphicsPipeline()->GetPipeline());
    }
