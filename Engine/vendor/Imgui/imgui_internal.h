...
//#endif
    ...
    // MUNI_CUSTOM begin
    IMGUI_API bool CustomTreeNodeWithPicBehavior(ImTextureID textureID, ImGuiID id, ImGuiTreeNodeFlags flags, const char* labelArg, const char* label_endArg);
    IMGUI_API int IndexOfCharWhereTheTextShouldBeCutToFit(const char* text, const char* textend, float size);
    IMGUI_API void HierarchyPanelTreePushOverrideID(ImGuiID id);
    IMGUI_API bool TextWithLineLimitV(const char* fmt, const int& lineLimit, va_list args);
    IMGUI_API bool TextWithLineLimitEx(const char* text, const int& lineLimit, const char* text_end_Arg, ImGuiTextFlags flags);
    IMGUI_API bool TextWithLineLimitAndToolTipV(const char* fmt, const int& lineLimit, va_list args);
    IMGUI_API bool StylisedButton(const char* label, const ImVec2& size_arg, ImDrawFlags drawFlags);
    IMGUI_API bool StylisedButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, ImDrawFlags drawFlags);
    IMGUI_API bool DragFloatRounded(const char* label, float* v, float v_speed, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, ImDrawFlags drawFlags);
    IMGUI_API bool CustomTreeNodeWithPicEx(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, ImTextureID textureID, ...);
    IMGUI_API bool CustomTreeNodeWithPicExV(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, ImTextureID textureID, va_list args);
